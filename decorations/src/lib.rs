//! Server-Side Decorations — titlebar, borders, buttons, shadows.

use labwc_core::Border;
use labwc_window::View;
use tracing::debug;

pub struct Ssd {
    pub view_id: u64,
    pub titlebar_height: i32,
    pub border_width: i32,
    pub margin: Border,
}

impl Ssd {
    pub fn create(view: &View) -> Self {
        let border = 1;
        let titlebar = 26;
        Self {
            view_id: view.id,
            titlebar_height: titlebar,
            border_width: border,
            margin: Border {
                left: border,
                right: border,
                top: border + titlebar,
                bottom: border,
            },
        }
    }

    pub fn get_margin(&self) -> Border {
        self.margin
    }
    pub fn destroy(&self) {
        debug!("SSD destroyed for view {}", self.view_id);
    }

    pub fn extents(&self) -> Border {
        self.margin
    }

    pub fn set_active(&self, active: bool) {
        debug!("SSD view {} active={}", self.view_id, active);
    }

    pub fn update_title(&self, title: &str) {
        debug!("SSD view {} title={}", self.view_id, title);
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use labwc_core::ViewType;
    use labwc_window::View;

    #[test]
    fn test_ssd_create() {
        let v = View::new(1, ViewType::XdgShell);
        let ssd = Ssd::create(&v);
        assert_eq!(ssd.view_id, 1);
        assert_eq!(ssd.titlebar_height, 26);
        assert_eq!(ssd.border_width, 1);
    }

    #[test]
    fn test_ssd_margin() {
        let v = View::new(1, ViewType::XdgShell);
        let ssd = Ssd::create(&v);
        let m = ssd.get_margin();
        assert_eq!(m.left, 1);
        assert_eq!(m.right, 1);
        assert_eq!(m.top, 27);
        assert_eq!(m.bottom, 1);
    }
}
