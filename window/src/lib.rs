...
use labwc_core::{
    Edge, IconState, LastPlacement, Rect, SsdMode, SsdPreference, ViewAxis, ViewLayer, ViewType,
};
...
/// Concrete view implementation kind — zero-cost dispatch via match.
...
impl View {
    pub fn new(id: u64, view_type: ViewType) -> Self {
        Self {
            id,
            view_type,
            impl_: None, // set by shell/xwayland after creation
            ...
            surface_id: AtomicU64::new(0),
        }
    }

    /// Lock ordering: fullscreen → maximized → tiled (consistent across crate).
    /// Deadlock-free as long as this order is maintained everywhere.
    ...
    fn surface_exists(&self) -> bool {
        self.surface_id.load(std::sync::atomic::Ordering::Relaxed) != 0
    }
    ...
    pub fn move_resize(&self, geo: Rect) {
        // Dispatch to the appropriate shell handler based on impl kind.
        // In the full compositor, this calls into the registered handler.
        // For now, configure directly updates pending geometry.
    ...
    pub fn maximize(&self, axis: ViewAxis) {
        // Maximize state updated directly — handler notified via callback
        *self.maximized.lock() = axis;
    }

    pub fn set_fullscreen(&self, fs: bool) {
        // Fullscreen state updated directly
    ...
    pub fn snap_to_edge(&self, edge: Edge, output: &Output) {
        if *self.fullscreen.lock() || edge.is_empty() || edge == Edge::CENTER {
            return;
        }
        *self.tiled.lock() = edge;
        ...
        let hw = (x2 - x1) / 2;
        let hh = (y2 - y1) / 2;
        let (nx, ny, nw, nh) = match () {
            _ if edge.contains(Edge::RIGHT | Edge::LEFT)
                && edge.contains(Edge::TOP | Edge::BOTTOM) =>
            {
                let left = if edge.contains(Edge::LEFT) {
                    x1
                } else {
                    x1 + hw
                };
                let top = if edge.contains(Edge::TOP) {
                    y1
                } else {
                    y1 + hh
                };
            ...
            _ if edge.contains(Edge::RIGHT) => (x1 + hw, y1, hw, y2 - y1),
            _ if edge.contains(Edge::LEFT) => (x1, y1, hw, y2 - y1),
            _ if edge.contains(Edge::BOTTOM) => (x1, y1 + hh, x2 - x1, hh),
            _ if edge.contains(Edge::TOP) => (x1, y1, x2 - x1, hh),
            _ => return,
        };
...
impl ViewManager {
    ...
    pub fn bottommost(&self) -> Option<&Arc<View>> {
        self.views.last()
    }
}
