...
pub fn autostart(ui: &mut egui::Ui, s: &mut AppState) {
    section(ui, "XDG Autostart");
    ui.label("Applications that start automatically with labwc-rs.");
    ui.label("Configured via XDG Autostart Specification.");
    ui.add_space(4.0);

    let total = s.autostart_entries.len();
    let enabled = s.autostart_entries.iter().filter(|e| e.enabled).count();
    ui.label(format!("{}/{} autostart entries enabled", enabled, total));
    ui.add_space(4.0);
    ui.separator();

    for item in s.autostart_entries.iter_mut() {
        ui.horizontal(|ui| {
            ui.checkbox(&mut item.enabled, "");
            ui.vertical(|ui| {
                ui.label(egui::RichText::new(&item.name).strong());
                ui.label(
                    egui::RichText::new(&item.exec)
                        .size(10.0)
    ...
    ui.add_space(8.0);
    if ui.button("Scan XDG Autostart Directories").clicked() {
        s.status_msg = "XDG autostart directories scanned".into();
    }
    if ui.button("Apply Autostart Changes").clicked() {
        s.status_msg = "Autostart configuration saved".into();
    }
}

pub fn about(_ui: &mut egui::Ui) {}
