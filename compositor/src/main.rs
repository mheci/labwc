...
fn send_signal(sig: i32) {
    // SAFETY: kill(2) with a validated PID and known signal number is safe.
    // We only send SIGHUP (reload) and SIGTERM (graceful shutdown).
    if sig != libc::SIGHUP && sig != libc::SIGTERM {
        error!("Refusing to send unknown signal: {sig}");
        return;
    }
