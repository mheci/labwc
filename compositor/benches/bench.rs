...
use labwc_animation::{Animation, Easing};
...
fn bench_rect(c: &mut Criterion) {
    ...
    c.bench_function("rect_intersection", |b| {
        b.iter(|| r1.intersection(black_box(&r2)))
    });
}
...
fn bench_animation(c: &mut Criterion) {
    let mut anim = Animation::new(0, 0.0, 100.0, 200, Easing::EaseInOut);
    c.bench_function("animation_tick", |b| b.iter(|| anim.tick()));
}
